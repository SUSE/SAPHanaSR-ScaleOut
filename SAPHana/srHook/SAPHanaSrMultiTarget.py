...
try:
    class SAPHanaSrMultiTarget(HADRBase):
        ...
        def __init__(self, *args, **kwargs):
            ...
            method = "init"
            episode = getEpisode()
            logTimestamp(self, method, episode, "init called")
            ...
            logTimestamp(self, method, episode, "pre call " + myCMD)
            rc = os.system(myCMD)
            logTimestamp(self, method, episode, "post call " + myCMD)
            ...
            if rc != 0:
                ...
                myMSG = "CALLING CRM: <{0}> rc={1}".format(myCMD, rc)
                self.tracer.info("{0}.{1}() {2}\n".format(self.__class__.__name__, method, myMSG))
            logTimestamp(self, method, episode, "init exit")
        ...
        def srConnectionChanged(self, ParamDict, **kwargs):
            ...
            episode = getEpisode()
            logTimestamp(self, method, episode, "srConnectionChanged called")
            ...
            logTimestamp(self, method, episode, "send dict message to log")
            ...
            if self.cib_access == "all-on" or self.cib_access == "glob-on" or self.cib_access == "site-off":
                ...
                myMSG = "CALLING CRM: <{0}> rc={1}".format(myCMD, rc)
                self.tracer.info("{0}.{1}() {2}\n".format(self.__class__.__name__, method, myMSG))
                logTimestamp(self, method, episode, "send result to log")
                self.tracer.info("{0}.{1}() Running srHookGeneration {2}, see attribute hana_{3}_gsh too\n".format(self.__class__.__name__, method, srHookGen, mysid))
                logTimestamp(self, method, episode, "differ cases following dictionary entries")
                ...
                if self.cib_access == "all-on" or self.cib_access == "site-on":
                    ...
                    if rc == 0:
                        ...
                        try:
                            ...
                            logTimestamp(self, method, episode, "new event - pending fallback file {0} deleted".format(fallback_file_name))
                        except FileNotFoundError:
                            pass
                    else:
                        logTimestamp(self, method, episode, "update cluster attribute failed, enter fallback")
                        #
                        # FALLBACK
                        ...
                        #
                        logTimestamp(self, method, episode, "prepare fallback attribute file (stage)")
                        ...
                        logTimestamp(self, method, episode, "created fallback attribute file (stage)")
                        #
                        ...
                        #
                        logTimestamp(self, method, episode, "move fallback attribute file stage to live")
                        os.rename(fallback_stage_file_name, fallback_file_name)
                        logTimestamp(self, method, episode, "moved fallback attribute file stage to live")
            logTimestamp(self, method, episode, "srConnectionChanged exit")
            return 0
