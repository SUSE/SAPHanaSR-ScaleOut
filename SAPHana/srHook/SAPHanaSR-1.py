...
class SAPHanaSR(HADRBase):  
    ...
    def postTakeover(self, rc, **kwargs):  
        ...
        if rc == 0:  
            ...
            return 0
    def srConnectionChanged(self, rc, **kwargs):
        """ finally we got the srConnection hook :) """
        self.tracer.info("%s.srConnectionChanged method called with rc=%s" % (self.__class__.__name__, rc))
        myHostname = socket.gethostname()
        myLogFile = "/hana/shared/srHook/srHook." + myHostname + ".log"
        myDatebase = rc["database"];
        mySystemStatus = rc["system_status"];
        ...
        with open(myLogFile, "a") as myfile:
            myfile.write("### %s.srConnectionChanged method called with rc=%s ###\n" % (self.__class__.__name__, rc))
            myfile.write("### SID=%s DB=%s SRS=%s ###\n" % (mysid, myDatebase, mySRS))
            myfile.write("### CALLING: <%s> rc=%s\n" % ( myCMD, rc ))
        return 0
